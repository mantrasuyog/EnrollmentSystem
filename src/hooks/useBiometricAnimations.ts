...
export const useBiometricAnimations = () => {
  ...
  useEffect(() => {
    ...
    faceScanlineAnimation.start();

    const fingerScanlineAnimation = Animated.loop(
      Animated.sequence([
    ...
    fingerScanlineAnimation.start();

    const faceFrameAnimation = Animated.loop(
      Animated.sequence([
    ...
    faceFrameAnimation.start();

    const fingerPulseAnimation = Animated.loop(
      Animated.sequence([
    ...
    return () => {
      faceScanlineAnimation.stop();
      fingerScanlineAnimation.stop();
      faceFrameAnimation.stop();
      fingerPulseAnimation.stop();
      waveAnimation1.stop();
      waveAnimation2.stop();
  ...
  const faceScanlineTranslateY = faceScanlineAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [-80, 80],
  });

  const fingerScanlineTranslateY = fingerScanlineAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [-60, 60],
  });

  const faceFrameOpacity = faceFrameAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0.3, 0.9],
  });

  const fingerPulseScale = fingerPulseAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0.8, 1.1],
  });
  ...
  const bubble1Transform = bubble1.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -height * 0.5],
  });

  const bubble2Transform = bubble2.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -height * 0.6],
  });

  const bubble3Transform = bubble3.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -height * 0.55],
  });

  const bubble4Transform = bubble4.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -height * 0.65],
  });

  const bubble5Transform = bubble5.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -height * 0.5],
  });
